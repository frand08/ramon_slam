...
float lastUpdate = 0, firstUpdate = 0, Now = 0;    // used to calculate integration interval
...
void mpu9250_init(void)
{
	...
	mpu9250.Ascale = AFS_16G;     // AFS_2G, AFS_4G, AFS_8G, AFS_16G
	...
	mpu9250.Mmode = 0x06;        // Either 8 Hz 0x02) or 100 Hz (0x06) magnetometer data ODR
	mpu9250.hi2c = hi2c1;
	mpu9250.deltat = 0.0f;                             // integration interval for both filter schemes
	mpu9250.q[0] = 1.0f;								// vector to hold quaternion
	mpu9250.q[1] = 0.0f;
	mpu9250.q[2] = 0.0f;
	mpu9250.q[3] = 0.0f;
	mpu9250.eInt[0] = 0.0f;		// vector to hold integral error for Mahony method
	mpu9250.eInt[1] = 0.0f;
	mpu9250.eInt[2] = 0.0f;
	mpu9250.beta = sqrt(3.0f / 4.0f) * GyroMeasError;  // compute beta
...
//	static float sum = 0;
	uint32_t transmit = 0;
	static float ax = 0, ay = 0, az = 0, gx = 0, gy = 0, gz = 0, mx = 0, my = 0, mz = 0; // variables to hold latest sensor data values
//	float temperature;
//	int16_t tempCount;   // Stores the real internal chip temperature in degrees Celsius
  ...
  // If intPin goes high, all data registers have new data
	if(mpu9250.readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01)  // On interrupt, check if data ready interrupt
	{
		...
		ax += (float)accelCount[0]*mpu9250.aRes;// - accelBias[0];  // get actual g value, this depends on scale being set
		ay += (float)accelCount[1]*mpu9250.aRes;// - accelBias[1];
		az += (float)accelCount[2]*mpu9250.aRes;// - accelBias[2];

		mpu9250.readGyroData(gyroCount);  // Read the x/y/z adc values
		// Calculate the gyro value into actual degrees per second
		gx += (float)gyroCount[0]*mpu9250.gRes;// - gyroBias[0];  // get actual gyro value, this depends on scale being set
		gy += (float)gyroCount[1]*mpu9250.gRes;// - gyroBias[1];
		gz += (float)gyroCount[2]*mpu9250.gRes;// - gyroBias[2];

		mpu9250.readMagData(magCount);  // Read the x/y/z adc values
		// Calculate the magnetometer values in milliGauss
		// Include factory calibration per data sheet and user environmental corrections
		mx += (float)magCount[0]*mpu9250.mRes*magCalibration[0];// - magbias[0];  // get actual magnetometer value, this depends on scale being set
		my += (float)magCount[1]*mpu9250.mRes*magCalibration[1];// - magbias[1];
		mz += (float)magCount[2]*mpu9250.mRes*magCalibration[2];// - magbias[2];
	...
	if(sumCount >= 30)
	{
		//Se calculan los promedios de las tres magnitudes
		ax = ax/sumCount;
		ay = ay/sumCount;
		az = az/sumCount;
...
//		sum += mpu9250.deltat;
		...
		//Ahora, pasamos la aceleracion a m/s^2
		imu->linear_acceleration.x = ax*GRAVITY;
		imu->linear_acceleration.y = ay*GRAVITY;
		imu->linear_acceleration.z = az*GRAVITY;
		//Luego, pasamos la velocidad angular a rad/sec
		imu->angular_velocity.x = gx*PI/180.0f;
		imu->angular_velocity.y = gy*PI/180.0f;
		imu->angular_velocity.z = gz*PI/180.0f;
...
//		temperature = ((float) tempCount) / 333.87f + 21.0f; // Temperature in degrees Centigrade
		...
		transmit = 1;	//Para avisarle que transmita
//		sum = 0;
		sumCount = 0;
		imu->orientation.w = mpu9250.q[0];
		imu->orientation.x = mpu9250.q[1];
		imu->orientation.y = mpu9250.q[2];
		imu->orientation.z = mpu9250.q[3];
	}
	return transmit;
}
...
{
	HAL_I2C_Mem_Write(&hi2c,address,subAddress,I2C_MEMADD_SIZE_8BIT,&data,1,100);
}

uint8_t MPU9250::readByte(uint8_t address, uint8_t subAddress)
{
	uint8_t data; // `data` will store the register data
	HAL_I2C_Mem_Read(&hi2c,address,subAddress,I2C_MEMADD_SIZE_8BIT,&data,1,100);
	return data;
}

void MPU9250::readBytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t * dest)
{
	HAL_I2C_Mem_Read(&hi2c,address,subAddress,I2C_MEMADD_SIZE_8BIT,&dest[0],count,100);
}
...
//	   writeByte(MPU9250_ADDRESS, INT_PIN_CFG, 0x22);
	   writeByte(MPU9250_ADDRESS, INT_PIN_CFG, 0x00);
